...
args=(
	...
	-no-user-config
	-cpu 'host,migratable=on,vmx=on,hv-time=on,hv-relaxed=on,hv-vapic=on,hv-spinlocks=0x1fff,hv-vpindex=on,hv-runtime=on,hv-synic=on,hv-stimer=on,hv-stimer-direct=on,hv-reset=on,hv-vendor-id=KVM Hv,hv-frequencies=on,hv-reenlightenment=on,hv-tlbflush=on,hv-ipi=on'
	-smp ${CPU}
	-m ${MEM}
	...
	-blockdev '{"driver":"file","filename":"'${SCRIPT_DIR}'/windows-11-new_VARS.fd","node-name":"libvirt-pflash1-storage","auto-read-only":true,"discard":"unmap"}'
	...
	-mem-prealloc
	-rtc base=localtime
	-drive file=/var/lib/libvirt/images/windows-11-new.img,if=virtio,format=raw,cache=writeback
	-enable-kvm
	...
	-netdev user,hostname=windows-11-new-user,id=nic
